...
impl ScarletData {
    ...
    fn get_consomation(cookies: Cookie) -> (f64, f64, Option<u32>) {
        ...
        let max_volume: f64 = max_volume_regex
            .captures(&html)
            .expect("Can't match max_volume_regex for max_volume")[1]
            .parse()
            .expect("Could not parse max_volume to f64");

        let transfert_volume: f64 = volume_regex
            .captures_iter(&html)
            .last()
            .expect("Can't match volume_regex for transfert_volume")[1]
            .replace(",", ".")
            .parse()
            .expect("Could not parse transfert volume to f64");

        let first_date_str = &first_date_regex
            .captures(&csv)
            .expect("Can't match first_date_regex")[0];

        println!("{:?}", first_date_str);

        let first_date = NaiveDate::parse_from_str(first_date_str, "%d/%m/%Y")
            .expect("Could not parse first_day as date");
        ...
        let days_left = last_date.signed_duration_since(Utc::now().naive_utc().date()).num_days();
